...
from actions.screen_processor import screen_process, screen_process_sync
...
def _log_traceback(prefix: str):
    log_path = os.environ.get("BRAHMA_LOG_FILE")
    try:
        print(prefix)
    except Exception:
        pass
...
def _load_system_prompt() -> str:
    try:
        ...
        return (
            "You are Brahma AI, an efficient assistant. "
...
TOOL_DECLARATIONS = [
        {
            ...
            "description": "Organizes files in the Downloads folder by type or date.",
            "parameters": {
                "type": "OBJECT",
                "properties": {
                    "mode": {"type": "STRING", "description": "by_type or by_date (default: by_type)"}
                },
                "required": []
            }
        },
    ...
    },
{
    ...
    },
    {
        ...
        "description": (
            ...
            "Use it to get a direct answer about what is visible."
        ),
        "parameters": {
            "type": "OBJECT",
            "properties": {
    ...
    },
    {
        ...
        "description": (
            "Creates, lists, shows, runs, and deletes named workflows so Brahma AI can automate routines "
            "like study mode, work mode, or creator setup."
        ),
        "parameters": {
            "type": "OBJECT",
            "properties": {
                "action": {"type": "STRING", "description": "create | list | show | run | delete"},
                "name": {"type": "STRING", "description": "Workflow name"},
                "steps": {"type": "ARRAY", "items": {"type": "STRING"}, "description": "Workflow steps"},
                "commands": {"type": "STRING", "description": "Steps as newline-separated text"},
                "description": {"type": "STRING", "description": "Fallback freeform workflow steps"},
            },
            "required": ["action"]
        }
    },
    {
        ...
        "description": (
            "Captures, shows, searches, and clears clipboard history. "
            "Use for clipboard history, recently copied text, or searching copied snippets."
        ),
        "parameters": {
            "type": "OBJECT",
            "properties": {
                "action": {"type": "STRING", "description": "show | search | capture | clear"},
                "query": {"type": "STRING", "description": "Search text for clipboard history"},
            },
            "required": ["action"]
        }
    },
    {
        "name": "system_health",
        "description": (
            "Reports live system health including CPU, memory, disk, battery, and optionally top processes."
        ),
        "parameters": {
            "type": "OBJECT",
            "properties": {
                "action": {"type": "STRING", "description": "summary | processes"},
            },
            "required": []
        }
    },
    {
        "name": "desktop_search",
        "description": (
            "Searches the user's files using natural language, such as finding PDFs, code files, "
            "or recently edited documents."
        ),
        "parameters": {
            "type": "OBJECT",
            "properties": {
                "query": {"type": "STRING", "description": "Natural language file search query"},
                "path": {"type": "STRING", "description": "Optional folder path to search inside"},
            },
            "required": ["query"]
        }
    },
    {
        "name": "kasa_control",
        "description": (
            "Discovers and controls TP-Link Kasa smart devices on the local network. "
            "Use for finding Kasa devices, turning them on or off, toggling them, or checking their status."
        ),
        "parameters": {
            "type": "OBJECT",
            "properties": {
                "action": {"type": "STRING", "description": "discover | on | off | toggle | status"},
                "device_name": {"type": "STRING", "description": "Friendly name of the Kasa device"},
            },
            "required": ["action"]
        }
    },
...
class JarvisLive:
    ...
    def submit_text(self, text: str):
        """Submit typed text as a user turn to the live session.

        Debounces identical messages sent within 1 second to avoid duplicates.
        If the live session isn't ready yet, queue the text to be sent once connected.
        """
        import time as _time

        now = _time.time()
        # simple debounce: ignore exact duplicates within 1s
        if text and text == self._last_sent_text and (now - self._last_sent_time) < 1.0:
            return
        ...
        try:
            ...
            try:
                self.ui.write_log("SYS: Failed to send typed message to the session")
            except Exception:
                pass
            _log_traceback("[error] Failed to send typed text to live session")
    ...
    async def _execute_tool(self, fc) -> types.FunctionResponse:
        ...
        print(f"[Brahma] 🔧 TOOL: {name}  ARGS: {args}")
        ...
        try:
            if name == "open_app":
                ...
                mode = args.get("mode", "by_type")
                from actions.file_controller import organize_downloads
                r = await loop.run_in_executor(
                    None, lambda: organize_downloads(mode)
                )
                result = r or "Downloads organized."
                ...
                try:
                    self.ui.start_screen_analysis()
                except Exception:
                    pass
                try:
                    r = await loop.run_in_executor(
                        None,
                        lambda: screen_process_sync(args, player=self.ui)
                    )
                    result = r or "I analyzed the image, but I couldn't determine a clear answer."
                finally:
                    try:
                        self.ui.stop_screen_analysis()
                    except Exception:
                        pass
                ...
                r = await loop.run_in_executor(
                    None, lambda: workflow_manager(parameters=args, player=self.ui)
                )
                result = r or "Workflow action completed."

            elif name == "clipboard_manager":
                r = await loop.run_in_executor(
                    None, lambda: clipboard_manager(parameters=args, player=self.ui)
                )
                result = r or "Clipboard action completed."

            elif name == "system_health":
                r = await loop.run_in_executor(
                    None, lambda: system_health(parameters=args, player=self.ui)
                )
                result = r or "System health checked."

            elif name == "desktop_search":
                r = await loop.run_in_executor(
                    None, lambda: desktop_search(parameters=args, player=self.ui)
                )
                result = r or "Desktop search completed."

            elif name == "kasa_control":
                r = await loop.run_in_executor(
                    None, lambda: kasa_control(parameters=args, player=self.ui)
                )
                result = r or "Kasa action completed."
    ...
    async def _listen_audio(self):
        ...
        try:
            while True:
                mic_enabled = True
                try:
                    mic_enabled = bool(self.ui.is_mic_enabled())
                except Exception:
                    mic_enabled = True
                ...
                async for response in turn:
                    ...
                    if response.server_content:
                        ...
                        if sc.input_transcription and sc.input_transcription.text:
                            ...
                            if txt:
                                in_buf.append(txt)
                                self.ui.set_live_user_text(" ".join(in_buf).strip())

                        if sc.output_transcription and sc.output_transcription.text:
                            txt = sc.output_transcription.text.strip()
                            if txt:
                                out_buf.append(txt)
                                self.ui.set_live_ai_text(" ".join(out_buf).strip())
                        ...
                        if sc.turn_complete:
                            ...
                            out_buf = []
                            self.ui.clear_live_transcripts()
                    ...
                    if response.tool_call:
                        ...
                        for fc in response.tool_call.function_calls:
                            print(f"[Brahma] 📞 Tool call: {fc.name}")
    ...
    async def run(self):
        ...
        if DISABLE_LIVE:
            # Keep backend available for offline/HTTP flows without opening a live session.
            self.ui.live_ready = False
            self.ui.status_text = "ONLINE"
            self.ui.write_log("SYS: Live streaming disabled; using offline assistant.")
            while True:
        ...
        while True:
            try:
                ...
                async with (
                    ...
                    self.ui.live_ready  = True
                    self.ui.status_text = "ONLINE"

                    # Flush any typed inputs queued before connection
                    if self._pending_inputs:
                        try:
                            for txt in list(self._pending_inputs):
                    ...
                    self.ui.write_log("Brahma AI online.")
                ...
                print(f"[Brahma] ⚠️  Error: {e}")
                try:
                    self.ui.write_log(f"[error] Live session error: {e}")
                except Exception:
                    pass
                try:
                    self.ui.live_ready = False
                    self.ui.status_text = "ERROR"
                except Exception:
                    pass
                _log_traceback("[error] Live session traceback")

            print("[Brahma] 🔄 Reconnecting in 3s...")
            try:
                self.ui.live_ready = False
            except Exception:
                pass
...
def main():
    ...
    def runner():
        ...
        try:
            ...
            print("\n🔴 Shutting down...")
            try:
                if hasattr(brahma, 'session') and brahma.session:
                    brahma.session.close()
            except Exception:
                pass
            try:
                if hasattr(ui, 'root'):
